...
impl Cdp {
    pub async fn connect(url: &str, timeout: Duration) -> anyhow::Result<Self> {
        ...
        let (socket, _) = connect_async_with_config(url, Some(config), false)
            .await
            .with_context(|| format!("connect CDP {url}"))?;
        Ok(Self {
            socket,
            next_id: 0,
            events: BTreeMap::new(),
    ...
    pub async fn send(&mut self, method: &str, params: Value) -> anyhow::Result<Value> {
        ...
        loop {
            let message = tokio::time::timeout(self.timeout, self.socket.next())
                .await
                .with_context(|| format!("CDP timeout: {method}"))?
    ...
    pub async fn wait_event(&mut self, name: &str) -> anyhow::Result<Value> {
        if let Some(value) = self.events.get_mut(name).and_then(|values| values.pop()) {
            return Ok(value);
        }
        loop {
            let message = tokio::time::timeout(self.timeout, self.socket.next())
                .await
                .with_context(|| format!("CDP event timeout: {name}"))?
                .context("CDP disconnected")??;
            let Message::Text(text) = message else {
                continue;
            };
