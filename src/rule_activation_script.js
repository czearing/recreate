  ...
  const flattenRules = (rules, media = null, gates = [], carriers = [], seen = new Set()) => {
    ...
    for (const rule of Array.from(rules || [])) {
      ...
      // A rule is not either a record or a container. CSS Nesting gives a style rule its own
      // declarations *and* child rules, so deciding both with one test loses whichever half
      // the test does not name. Which half depends on the CSSOM generation the browser
      // ships: `CSSStyleRule` began inheriting `CSSGroupingRule` in Chrome 130, so a walk
      // that asks about grouping first records every style rule on one engine and none on
      // the next. Asking the narrower question first makes that inheritance unobservable.
      // A style rule states no condition, so its children inherit its gates and carriers
      // exactly, and its own selector is what composes theirs.
      if (rule instanceof CSSStyleRule) {
        entries.push({ rule, media, gates, carriers, active: true });
        entries.push(...flattenRules(rule.cssRules, media, gates, carriers, seen));
        continue;
      }
      // `@keyframes` is neither: it exposes children, but they are keyframe selectors rather
      // than rules the cascade resolves, so its steps are still never descended into.
      if (!grouping(rule)) {
        entries.push({ rule, media, gates, carriers, active: true });
        continue;
      }
  ...
  const activateEntries = entries => {
    ...
    for (const entry of entries) {
      const subject = subjectOf(entry.rule);
      if (!entry.gates.length || !subject) continue;
      const selector = staticSelector(subject);
