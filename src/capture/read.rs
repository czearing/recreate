...
pub async fn read_visual_interaction_state(
    cdp: &mut crate::cdp::Cdp,
    viewport: Viewport,
) -> Result<PageState> {
    read(
        cdp,
        viewport,
        &crate::interaction_script::visual_source(),
        "visual interaction capture returned non-string",
    )
    .await
}
