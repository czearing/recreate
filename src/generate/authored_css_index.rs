...
impl<'a> Index<'a> {
    pub fn new(rules: &'a [String]) -> Self {
        ...
        for rule in rules {
            ...
            if selectors.starts_with('@') || selectors.contains(':') {
                continue;
            }
    ...
    pub fn declarations(&self, node: &Node) -> Styles {
        ...
        for index in self.direct_indices(node) {
            ...
            for (name, value) in parsed_declarations(rule.declarations).filter(|(name, value)| {
                super::authored_css_rules::retained(name) && !value.contains("var(")
            }) {
        ...
        values
            ...
            .filter_map(|(name, values)| {
                values
                    .iter()
                    .rev()
                    .find(|value| super::authored_css_rules::resolved_matches(node, &name, value))
    ...
    pub fn has_property(&self, node: &Node, property: &str) -> bool {
        self.direct_indices(node).into_iter().any(|index| {
            parsed_declarations(self.rules[index].declarations).any(|(name, _)| name == property)
        })
    }
