use super::scroll_state::{self, Scrolled};
    ...
    interaction.states.iter().any(|state| {
        baselines
            ...
            .is_some_and(|baseline| {
                scroll_state::changed(baseline, state)
                    .iter()
                    .any(|scrolled| {
                        scrolled.left != 0 && !scroll_state::scrolls_document(state, scrolled.path)
                    })
            })
    })
}

/// Serializes the scroll the action produced, splitting the one element the runtime reaches
/// through the global `scrollTo` from the ones it reaches by `querySelector`.
fn scroll_snapshot(baseline: &PageState, state: &PageState) -> String {
    let scrolled = scroll_state::changed(baseline, state);
