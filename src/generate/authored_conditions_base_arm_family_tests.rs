...
use super::{decided, divisions, node, restored, scene};
...
fn a_family_that_divides_by_edge_returns_to_its_own_base_arm() {
    let card = decided(
        node("card", &[("margin-top", "20px"), ("margin-left", "30px")]),
...
fn a_base_arm_the_engine_could_not_settle_deletes_nothing() {
    let card = decided(node("card", &[("padding-top", "40px")]), &["padding-top"]);
    let rules = vec![
...
fn an_artifact_carrying_no_division_withdraws_nothing_and_deletes_nothing() {
    let wide = decided(
        node("multi", &[("background-color", "rgb(0, 255, 0)")]),
        &["background-color"],
    );
