...
pub fn app(
    ...
    let output = output.replace(
        "const activate=(event,next)=>{lastTrigger.current=event.currentTarget.dataset.recreateTrigger;",
        "const activate=(event,next)=>{const previousTrigger=lastTriggerElement.current;previousTrigger?.removeAttribute('data-recreate-active');lastTriggerElement.current=event.currentTarget;event.currentTarget.dataset.recreateActive='true';lastTrigger.current=event.currentTarget.dataset.recreateTrigger;const[,command]=reduceInteraction({openSurface:state||null,activeTrigger:previousTrigger},{type:'activate',trigger:event.currentTarget,surface:next,closable:closableStates[next]});",
    );
    let output = output
        .replace("if(!closableStates[next]){", "if(command.type==='invoke'){")
        .replace(
